...
public class Battery extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        user.getItemCooldownManager().set(this, BASE_COOLDOWN_TICKS + chargeTime);
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (remainingUseTicks == chargeTime - 8) {
            SoundEvent sound = this.shield >= 20 ? RPSoundEvents.BATTERY_CHARGE : RPSoundEvents.CELL_CHARGE;
            world.playSound(null, user.getX(), user.getY(), user.getZ(),
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (user instanceof ServerPlayerEntity player) {
            ServerPlayNetworking.send(player, new SoundInterruptS2CPayload(this.shield >= 20 ? 0 : 1));
